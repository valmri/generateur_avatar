...
include 'HabboAPI/vendor/autoload.php';

// Import de la bibliothèque HabboAPI
...
$habboParser = new HabboParser('fr');
...
if (isset($_POST["pseudo"]) && isset($_POST["hotel"])){                             // Contrôle et récupération des valeurs de "pseudo" et "hotel"
    if ($_POST["pseudo"]!=="") {                                                    // Vérification de la valeur saisie par l'utilisateur
        ...
        $verifPseudo = $habboApi->getHabbo($pseudo);

        var_dump($verifPseudo);

        // TODO : Créer un système de vérification via la patch HabboAPI qui est solution plus pérenne que HabboApi.net

/* Ancienne vérifiction
    $api = file_get_contents("http://api.habboapi.net/habbos?name=".$pseudo);   // Initialisation de la variable $api 
